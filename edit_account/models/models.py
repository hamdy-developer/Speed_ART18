...
class AccountMove(models.Model):
    ...
    def action_post(self):
        for move in self:
            if move.move_type == 'out_invoice' and move.partner_id.sudo().credit_limit > 0:
                ...
                allowed_credit = move.partner_id.sudo().credit_limit - move.partner_id.sudo().credit
...
class SaleOrder(models.Model):
    ...
    def action_confirm(self):
        for order in self:
            if order.partner_id.sudo().credit_limit > 0:

                allowed_credit = order.partner_id.sudo().credit_limit - order.partner_id.sudo().credit
